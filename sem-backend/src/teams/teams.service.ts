...
export class TeamsService {
  ...
  async createTeam(workspaceId: string, dto: CreateTeamDto, userId: string): Promise<Team> {
    await this.workspacesService.ensurePermission(workspaceId, userId, 'team.manage');
  ...
  async removeTeam(workspaceId: string, teamId: string, userId: string): Promise<void> {
    ...
    const playerUserIds = await this.workspacesService.getTeamPlayerUserIds(teamId);
    await this.workspacesService.sendNotificationToMany(
      playerUserIds,
